...
async function getSeoData(){
     return (await fetch(baseURL + "/pages/1?populate=*")).json()
}
async function getPageData(){
return await (await fetch(baseURL + "/pages/1?populate[ContentBlocks][populate]=*")).json()
   
}
async function getData(){
return await (await fetch(baseURL + "/pages/1?populate[PageLeft][populate]=*")).json()
   
}
async function getRightBlock(){
return await (await fetch(baseURL + "/pages/1?populate[PageRight][populate]=*")).json()
   
}
async function getfeaturesData(){
return await (await fetch(baseURL + "/pages/1?populate[Features][populate]=*")).json()
...
const leftblock = use(getData());
const rightblock = use(getRightBlock());
...
const featuresdata = use(getfeaturesData());
  return (
    <>
      ...
      <LeftBlock block={leftblock.data.attributes} />
      <RightBlock block={rightblock.data.attributes} />
